...
# --- PARAMETERS ---
VOL_5MIN_THRESHOLD_CR = 40.0 # High threshold!
...
def fetch_and_build_list():
    ...
    try:
        resp = requests.get(inst_url, headers=headers)
        ...
        # Clean Leverage Sheet
        lev_df["Symbol"] = lev_df.iloc[:, 1].astype(str).str.upper().str.strip()
...
def on_message(ws, message):
    ...
    try:
        # Full Packet (Code 8) is 50 bytes
        if response_code == 8 and len(message) >= 48:
    ...
    if len(volume_history[sec_id]) > 1:
        ...
        if traded_value_cr >= VOL_5MIN_THRESHOLD_CR:
            if now - alert_cooldowns.get(sec_id, 0) > COOLDOWN_SECONDS:
                ...
                msg = f"🚀 *VOL SPIKE: {symbol}*\nValue: ₹{traded_value_cr:.2f} Cr\nPrice: ₹{ltp}\nQty: {int(SIGNAL_AMOUNT/ltp) if ltp > 0 else 0}"
                print(msg.replace('*',''))
...
def heartbeat():
    while True:
        time.sleep(60)
        print(f"💓 Heartbeat: {datetime.now(IST).strftime('%H:%M:%S')} | Packets: {packets_received} | Monitored: {len(SIDS_LIST)}")
...
if __name__ == "__main__":
    ...
    if SIDS_LIST:
        print(f"✅ Subscribed to {len(SIDS_LIST)} stocks. Limit: {VOL_5MIN_THRESHOLD_CR} Cr")
        while True:
